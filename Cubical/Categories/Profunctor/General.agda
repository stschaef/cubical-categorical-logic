...
--
  ...
  UniqueFunctorComprehension isUnivD ues =
    ...
    ΣPathP (
      Functor≡
        (λ c → {! (TODO G universalAtG isUnivD) !} )
        {!!} ,
      -- UniversalElementOnToPshFunctorRepresentation F universalAtF) ,
    ...
    TODO G universalAtG isUnivD =
      NatIsoToPath (isUnivalentFUNCTOR (D ^op) (SET _) isUnivalentSET)
       (
         seqNatIso
           (symNatIso (UniversalElementOnToPshFunctorRepresentation F universalAtF))
