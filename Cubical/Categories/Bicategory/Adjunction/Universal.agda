...
{- A right adjoint to `f : c → d` as a universal element, one probe
   0-cell at a time.  `⟨ f ⟩ e` is postcomposition with `f`, read off
   the representable prestack `Hom B d` at the element `f : p[ c ]`;
   `RPre` is `reindPsh` of the representable presheaf along it, so
   this is `Cubical.Categories.Adjoint.RightAdjoint` in every fibre. -}
  ...
  module _ {c d : B.0Cell} (f : B.1Cell c d) where
    ...
    module _ {u : B.1Cell d c} (ε : B.2Cell (u B.⋆₁ f) B.id₁) where
      ...
      isRightAdjointᴮ = (e : B.0Cell) (y : B.1Cell e d)
        → isUniversal B.Hom[ e , c ] (RPre e y) (y B.⋆₁ u) (εEl y)
...
module RightAdjointᴮNotation {B : Bicategory ℓ ℓ' ℓ''}
  ...
  ΨFormula {e} {y} {x} θ =
    cong (Ψ {e} {y} {x}) (sym (Φfml θ)) ∙ retIsEq (U e y x) _

  private
    eqnR : ηw B ηU ε u B.⋆₂ (ε B.▷w u) ≡ B.λ⁻ u
    eqnR = sym (ΨFormula {y = B.id₁} {x = u} ε)
         ∙ cong (Ψ {y = B.id₁} {x = u}) (sym lemΦλ)
         ∙ retIsEq (U d B.id₁ u) (B.λ⁻ u)
  ...
  transposeFaithful {e} {x} {y} φ φ' p =
      sym (retIsEq (U e y x) φ)
    ∙ cong (invIsEq (U e y x)) p
    ∙ retIsEq (U e y x) φ'
  ...
  transposeIsoU {e} {x} {y} = invIso (equivToIso (_ , U e y x))
  ...
  whiskerZigzagRU {e} y =
      sym (ΨFormula (εB y))
    ∙ cong (Ψ {x = y B.⋆₁ u}) (sym (B.⟨ B.▷wId f ⟩⋆₂⟨⟩ ∙ B.⋆₂IdL _))
    ∙ retIsEq (U e y (y B.⋆₁ u)) B.id₂
...
module _ {B : Bicategory ℓ ℓ' ℓ''} {c d : Bicategory.0Cell B}
  ...
  AdjunctionAt≅RightAdjointᴮ .Iso.ret t@(u , ε , η , zl , zr) =
    ...
    module N = RightAdjointᴮNotation {B = B} {c = c} {d = d}
                 {f = f} {u = u} {ε = ε} Ut
    ηeq : N.ηU ≡ η
    ηeq = cong (invIsEq (Ut c f B.id₁))
               (sym (AdjunctionNotation.zigzagL⁺ (toAdj t)))
        ∙ retIsEq (Ut c f B.id₁) η
